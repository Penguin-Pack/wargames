...
import (
	...
	"github.com/Penguin-Pack/wargames/internal"
...
func getImage(username string) string {
	...
	postBody, _ := json.Marshal(map[string]string{
		"username": username,
		"apiToken": internal.GetEnv("API_TOKEN"),
	})

	responseBody := bytes.NewBuffer(postBody)

	resp, err := goHttp.Post(internal.GetEnv("API_URI")+"/image", "application/json", responseBody)
...
func verifyUser(username string) bool {
	...
	resp, err := goHttp.Post(internal.GetEnv("API_URI")+"/stats", "application/json", responseBody)
...
func (c *configHandler) OnConfig(request config.Request) (config.AppConfig, error) {
	...
	cfg.Docker.Execution.DockerLaunchConfig.ContainerConfig.Image = getImage(request.Username)
